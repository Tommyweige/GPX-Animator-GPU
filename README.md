...
- GPX 1.0/1.1, multiple segments, malformed-point validation, stationary/red-
  ...
  statistics.
- Satellite, light, dark and transparent map styles. Tiles are stored in a
  persistent disk LRU cache and missing network tiles use a parent/placeholder
  fallback instead of black blocks.
...
- English and Traditional Chinese UI with persisted preferences.
- Right-docked nearby places inspector. Right-click the map, preview a result
  as a Relive-style pin, then add it to the route without changing timeline
  progress. Added pins use the same renderer in preview and export.
...
- **Preload is slow:** the first export downloads missing tiles; later exports
  use the AppData cache. The preflight counter separates cache and network work.
...
- 衛星、明亮、深色、透明地圖；地圖磚會保存到本機 LRU cache，失敗時使用
  父層磚或可重現的 placeholder，避免黑色方塊。
- 只保留「跟隨」與「完整」輸出視角。跟隨視角使用真正 Web Mercator zoom，
  預覽滾輪不會切成自由視角；拖曳只改暫時預覽。
- 16:9、1:1、9:16、HUD、海拔圖，以及預設 8 px 路線。
- 右鍵地圖開啟固定右側的附近地點面板；先預覽圖針，再加入路線，不會改變
  路線進度。圖針在預覽與輸出使用同一個水滴樣式。
