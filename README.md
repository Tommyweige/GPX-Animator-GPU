...
- GPX 1.0/1.1, multiple segments, malformed-point validation, stationary/red-
  ...
  statistics.
- Satellite, light and transparent map styles. Transparent uses the same
...
- **Preload is slow:** the first export downloads missing tiles; later exports
  use the AppData cache. Light and Transparent share the OSM cache, while
...
- 衛星、明亮、淡化地圖；淡化地圖仍有真實 OSM 圖磚並以 35% 透明度疊在中性
  背景上（MP4 不支援 Alpha）。地圖
