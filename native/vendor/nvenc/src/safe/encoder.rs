...
use crate::sys::structs::{
    Guid, NV_ENC_CREATE_INPUT_BUFFER_VER, NV_ENC_EVENT_PARAMS_VER, NV_ENC_LOCK_INPUT_BUFFER_VER,
    NVencCodecPicParams, NVencCreateInputBuffer, NVencEventParams, NVencLockInputBuffer,
    NVencPicParamsH264, NVencRegisterResource,
};
use crate::sys::{
...
impl Encoder {
    ...
    pub fn encode_picture_async<I: EncoderInput>(
        &self,
        input: &I,
        output: &BitStream,
        frame_count: usize,
    ...
    pub fn end_encode_async(&self, completion_event: *mut c_void) -> Result<(), NVencError> {
        self.encoder.end_encode(completion_event)
    }
    pub fn register_async_event(&self, completion_event: *mut c_void) -> Result<(), NVencError> {
        self.encoder.register_async_event(completion_event)
    }
    pub fn unregister_async_event(&self, completion_event: *mut c_void) -> Result<(), NVencError> {
        self.encoder.unregister_async_event(completion_event)
    }
...
impl EncoderInternal {
    ...
    pub fn encode_picture<I: EncoderInput>(
        ...
        unsafe { &mut *params.as_mut_ptr() }.completion_event = completion_event;
        match unsafe {
    ...
    pub fn end_encode(&self, completion_event: *mut c_void) -> Result<(), NVencError> {
        let mut params: MaybeUninit<NVencPicParams> = MaybeUninit::zeroed();
        unsafe { &mut *params.as_mut_ptr() }.version = NV_ENC_PIC_PARAMS_VER;
    ...
    pub fn register_async_event(&self, completion_event: *mut c_void) -> Result<(), NVencError> {
        ...
        unsafe {
            (self.function_list.nvenc_register_async_event)(self.encoder.as_ptr(), &raw mut params)
        }
        .into_error()
    }
    pub fn unregister_async_event(&self, completion_event: *mut c_void) -> Result<(), NVencError> {
        ...
        params.completion_event = completion_event;
        unsafe {
            (self.function_list.nvenc_unregister_async_event)(
                self.encoder.as_ptr(),
                &raw mut params,
            )
        }
