...
impl NVencRcParams {
    /// Configures VBR constant-quality mode without exposing ABI-sensitive fields.
    pub fn configure_vbr_quality(
        &mut self,
        target_quality: u8,
        max_bit_rate: u32,
        spatial_aq: bool,
    ) {
        self.rate_control_mode = NVencParamsRcMode::VBR;
        self.max_bit_rate = max_bit_rate;
        self.target_quality = target_quality;
        self.target_quality_lsb = 0;
        self.look_ahead_depth = 0;
        const ENABLE_AQ_BIT: u32 = 1 << 3;
        if spatial_aq {
            self.bit_fields.0 |= ENABLE_AQ_BIT;
        } else {
            self.bit_fields.0 &= !ENABLE_AQ_BIT;
        }
    }
    ...
    pub fn target_quality(&self) -> u8 {
        self.target_quality
    }
    pub fn spatial_aq_enabled(&self) -> bool {
        self.bit_fields.0 & (1 << 3) != 0
    }
}
